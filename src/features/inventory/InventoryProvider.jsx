...
export default function InventoryProvider({ children }) {
 ...
 useEffect(()=>{Promise.resolve().then(refresh).catch(e=>setError(e.message))},[refresh])
 ...
 return <InventoryContext.Provider value={{products,adminProducts:[...products,...unavailable],setAvailability:(id,active)=>mutation(()=>api('/products/'+id+'/availability',{method:'PATCH',body:{active}})),categories:categoryRows.map(c=>c.name),movements,refresh,
