...
export default function QuotationsPage(){
 const {quotations:rows,setQuotations:setRows,convertQuotation,settings}=useDemo()
 const [query,setQuery]=useState(''),[dialog,setDialog]=useState(null),[notice,setNotice]=useState('')
 const active=dialog
 function close(){setDialog(null)}
 async function save(value){await setRows(current=>current.map(q=>q.id===value.id?value:q));setDialog(current=>({...current,entry:value}))}
 async function convert(q){await convertQuotation(q)}
