...
export default function CreateDocumentPage({kind}) {
 ...
 const {customers,settings,setInvoices,setQuotations}=useDemo(),{products}=useInventory(),navigate=useNavigate()
 const [lines,setLines]=useState([{id:'first',name:'',quantity:1,price:0}])
 const totals=calculateTotals(lines,settings.tax)
 function save(e){e.preventDefault();const data=Object.fromEntries(new FormData(e.currentTarget));const setter=invoice?setInvoices:setQuotations;setter(current=>[{...data,id:(invoice?'INV-':'QUO-')+crypto.randomUUID().slice(0,8).toUpperCase(),date:new Date().toISOString().slice(0,10),lines,items:lines.reduce((n,l)=>n+Number(l.quantity),0),amount:totals.total,taxRate:settings.tax,currency:settings.currency,status:'Pending'},...current]);navigate('/admin/'+path)}
 ...
 <form onSubmit={save} className="space-y-6"><section className="rounded-xl border border-outline p-6"><h2 className="mb-5 text-lg font-semibold">Customer Information</h2><div className="grid gap-5 md:grid-cols-2"><Field label="Customer Name *"><select name="customer" className={inputClass} required defaultValue=""><option value="" disabled>Select customer</option>{customers.map(c=><option key={c.id}>{c.name}</option>)}</select></Field><Field label={invoice?'Due Date *':'Valid Until *'}><input className={inputClass} name="due" type="date" min={new Date().toISOString().slice(0,10)} required/></Field></div></section>
