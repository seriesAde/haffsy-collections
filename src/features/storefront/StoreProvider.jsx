...
export default function StoreProvider({children}){
 const {products}=useInventory();const [cart,setCart]=useState(()=>{try{return JSON.parse(sessionStorage.getItem('store-cart')||'[]')}catch{return []}}),[profile,setProfile]=useState({name:'',email:'',phone:'',address:''}),[history,setHistory]=useState([]),[notice,setNotice]=useState('')
 useEffect(()=>{sessionStorage.setItem('store-cart',JSON.stringify(cart))},[cart])
 const {wishlist,toggleWish,wishlistLoading,wishlistError}=useWishlist(setNotice)
 ...
 return <StoreContext.Provider value={{cart,items,subtotal,add,quantity,remove:id=>setCart(rows=>rows.filter(r=>r.id!==id)),clear:()=>setCart([]),wishlist,toggleWish,wishlistLoading,wishlistError,profile,setProfile,history,setHistory,notice,setNotice}}>{children}</StoreContext.Provider>
}
