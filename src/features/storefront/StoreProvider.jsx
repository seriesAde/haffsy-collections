...
export default function StoreProvider({children}){
 const {products}=useInventory();const [cart,setCart]=useState(()=>{try{return JSON.parse(sessionStorage.getItem('store-cart')||'[]')}catch{return []}}),[wishlist,setWishlist]=useState([]),[profile,setProfile]=useState({name:'',email:'',phone:'',address:''}),[history,setHistory]=useState([]),[notice,setNotice]=useState('')
